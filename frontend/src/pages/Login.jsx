...
// users.csv dosyasındaki gerçek verilere göre güncellendi
const TEST_ACCOUNTS = [
  { tc: '11111111111', role: 'doctor', label: 'Doktor' },
  { tc: '22222222222', role: 'pharmacist', label: 'Eczacı' },
...
export default function Login({ onLoginSuccess }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Giriş bilgileri hatalı.');
      }
  ...
  const fillTestAccount = (tc) => {
    setTcNo(tc);
    setPassword('test1234');
    setError('');
  };

  // Mavi menüdeki tıklanabilir alanlar
  const panelItems = [
    { icon: Activity, label: 'Hekim Paneli', tc: '11111111111' },
    { icon: Pill, label: 'Eczacı Paneli', tc: '22222222222' },
    { icon: HeartPulse, label: 'Hasta Portalı', tc: '12345678901' },
  ];
      ...
      <div className="relative z-10 w-full max-w-5xl bg-white/70 backdrop-blur-2xl rounded-[2rem] shadow-[0_20px_70px_-15px_rgba(2,132,199,0.25)] overflow-hidden flex flex-col md:flex-row border border-white/80">
        ...
        <div className="relative w-full md:w-5/12 bg-gradient-to-br from-sky-700 via-sky-800 to-blue-900 p-10 text-white flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative mt-12 space-y-3">
            {panelItems.map(({ icon: Icon, label, tc }) => (
              <button
                key={label}
                type="button"
                onClick={() => fillTestAccount(tc)}
          ...
          <div className="max-w-sm mx-auto w-full">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 active:scale-[0.99] text-white font-semibold py-3 px-4 rounded-xl shadow-lg shadow-sky-600/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-sky-600 mt-1"
              >
                {isLoading ? (
                  <>
              ...
              <div className="grid grid-cols-3 gap-2">
                {TEST_ACCOUNTS.map((acc) => (
                  <button
                    key={acc.tc}
                    type="button"
                    onClick={() => fillTestAccount(acc.tc)}
                    className="flex flex-col items-center gap-1 px-2 py-2.5 rounded-xl bg-slate-50 hover:bg-sky-50 border border-slate-200 hover:border-sky-200 transition-all group"
                  >
